...
void BattlegroundSA::AddPlayer(Player *plr)
{
    ...
    if (!ShipsStarted)
    {
        ...
        {
           /* Neni potreba, spawn opraven plr->CastSpell(plr, 12438, False);//Without this player falls before boat loads...     */

            if (urand(0, 1))
                plr->TeleportTo(607, 2682.936f, -830.368f, 14.6660f, 2.895f, 0);
            else
                plr->TeleportTo(607, 2577.003f, 980.261f, 13.741f, 0.807f, 0);

        }
        else
            plr->TeleportTo(607, 1209.7f, -65.16f, 30.1f, 0.0f, 0);
    }
    else
    {
        if (plr->GetTeamId() == Attackers)
    ...
    for (BattlegroundPlayerMap::const_iterator itr = GetPlayers().begin(); itr != GetPlayers().end(); ++itr)
    {
        if (Player *plr = sObjectMgr->GetPlayer(itr->first))
        {
            ...
            if (plr->GetTeamId() == Attackers)
            {
                ...
                if (urand(0, 1))
                    plr->TeleportTo(607, 2682.936f, -830.368f, 14.666f, 2.895f, 0);
                else
                    plr->TeleportTo(607, 2577.003f, 980.261f, 13.741f, 0.807f, 0);
            }
            else
