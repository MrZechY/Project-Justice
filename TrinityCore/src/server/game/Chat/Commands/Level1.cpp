...
bool ChatHandler::HandleVipBuffCommand(const char* args)
{
    ...
    if (chr->isInCombat())
    {
        SendSysMessage("Tento prikaz nemuzes pouzit v combatu!");
        SetSentErrorMessage(true);
        return false;
    }
    if (chr->isInFlight())
    {
        SendSysMessage("Tento prikaz nemuzes pouzit kdyz letis!");
        SetSentErrorMessage(true);
        return false;
    }
    if (chr->InArena())
    {
        SendSysMessage("Tento prikaz nemuzes pouzit v Arene!");
        SetSentErrorMessage(true);
        return false;
    }
